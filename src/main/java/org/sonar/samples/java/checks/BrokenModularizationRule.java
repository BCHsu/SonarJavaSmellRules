			...
			if (classNode.haveSmell(smell)) {
				...
				for (InputFile javaFile : javaFiles) {
					...
					NewIssueLocation brokenModularizationLocation = brokenModularizationIssue.newLocation().on(javaFile)
							.at(javaFile.selectLine(issueStartLine)).message("Broken Modularization Location!");
					...
					if (classNode.getDataClass() != null) {
						...
						String dataClassMessage = "Data Class detected" + "\r\n" + "WOC: "
								+ ((Woc) ((DataClass) classNode.getDataClass()).getWoc()).getValue() + "\r\n" + "WMC: "
								+ ((Wmc) ((DataClass) classNode.getDataClass()).getWmc()).getValue() + "\r\n"
								+ "NOPA + NOAM: "
								+ ((NopaAndNoam) ((DataClass) classNode.getDataClass()).getNopaAndNoam()).getValue()
								+ "\r\n";
						NewIssueLocation dataClassLocation = brokenModularizationIssue.newLocation().on(javaFile)
								.at(javaFile.selectLine(classStartLine)).message(dataClassMessage);
					...
					for (MethodNode method : methods) {
						if (method.getFeatureEnvy() != null) {
							...
							String featureEnvyMessage = "Feature Envy detected" + "\r\n" + "ATFD: "
									+ ((Atfd) ((FeatureEnvy) method.getFeatureEnvy()).getAtfd()).getMetricValue()
									+ "\r\n" + "LAA: "
									+ ((Laa) ((FeatureEnvy) method.getFeatureEnvy()).getLaa()).getMetricValue() + "\r\n"
									+ "FDP: "
									+ ((Fdp) ((FeatureEnvy) method.getFeatureEnvy()).getFdp()).getMetricValue()
									+ "\r\n";
							NewIssueLocation featureEnvyLocation = brokenModularizationIssue.newLocation().on(javaFile)
									.at(javaFile.selectLine(methodStartLine)).message(featureEnvyMessage);
...
//				int classComplexity = context.getComplexityNodes((ClassTree) tree).size();
//				classNode.setWMC(classComplexity);
//				File file = context.getFile();
//				classNode.setFile(file);
//				classes.add(classNode);
//				String filePath = "D:\\test\\123.txt";
//				try {
//					BrokenModularizationRule.logOnFile(filePath, log);
//				} catch (ClassNotFoundException e) {
//					e.printStackTrace();
//				}
//			}
